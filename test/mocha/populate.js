...
var error = function(err, element) {
    if (err)
        console.log("Error while saving " + element + ": " + err);
};

function filterFromSchema(data, schema, noModify) {
    var res = {};
    var camelized;
    for (var el in data) {
        if (!noModify)
            camelized = camelize(el);
        else
            camelized = el;
...
function retrieveVariantDetail(data) {
    var detail = filterFromSchema(data, VariantDetail.schema);
...
function retrievePathogenicity(data) {
    var path = filterFromSchema(data, Pathogenicity.schema);
...
function parse(json) {
    ...
    json[key].forEach(function(element) {
        ...
        //build model classes
        var variant = new Variant(filterFromSchema(element, Variant.schema));
        var detail = new VariantDetail(retrieveVariantDetail(element));
        var gene = new Gene(filterFromSchema(element, Gene.schema));
        var dbsnp = new DbSNP(filterFromSchema(element, DbSNP.schema));
        var pathogenicity = new Pathogenicity(retrievePathogenicity(element));
        var esp = new Esp(filterFromSchema(element, Esp.schema, true));
...
var str = JSON.stringify(filterOnlyAttributes(parse(json)));
console.log(str);
